...
namespace POC.MVC.Controllers
{
    public class ProductController : Controller
    {
        ...
        [HttpGet]
        public async Task<IActionResult> Products(int page = 1 , string searchName ="" , string Category ="")
        {
            ...
            try
            {
                var cartCount = await SendAuthorizedRequestAsync(HttpMethod.Get, $"Cart/Cart?id={userId}");
                if (cartCount.IsSuccessStatusCode)
                {
                    var data = await cartCount.Content.ReadAsStringAsync();
                    int productCount = 0;
